...
{{-- Paleta de comandos (⌘K / Ctrl+K). Búsqueda difusa sobre `items`; Enter navega,
     ↑/↓ mueven, Escape cierra. Combobox + listbox: la opción activa se anuncia con
     aria-activedescendant. Tab queda atrapado en el diálogo y al cerrar el foco vuelve
     a donde estaba. --}}
<div
    x-data="{
        open:false, q:'', active:0, prevFoco:null,
        items: {{ $itemsJson }},
        init(){ this.$watch('open', v => { if(!v) this.devolverFoco(); }); },
        get results(){
            const t=this.q.trim().toLowerCase();
            if(!t) return this.items;
            return this.items.filter(i => (i.label+' '+(i.group||'')).toLowerCase().includes(t));
        },
        show(){ if(!this.open) this.prevFoco=document.activeElement; this.open=true; this.q=''; this.active=0; this.$nextTick(()=>this.$refs.input && this.$refs.input.focus()); },
        devolverFoco(){ const e=this.prevFoco; this.prevFoco=null; if(e && e.isConnected && e.focus && e!==document.body) e.focus(); },
        move(d){ const n=this.results.length; if(!n) return; this.active=(this.active+d+n)%n;
            this.$nextTick(()=>{ const o=document.getElementById(this.$id('muni-cp', 'o'+this.active)); o && o.scrollIntoView({block:'nearest'}); }); },
        go(){ const r=this.results[this.active]; if(r && r.url) window.location.href=r.url; },
        atrapar(ev){
            if(ev.key!=='Tab') return;
            const f=[...this.$refs.panel.querySelectorAll('a[href],button,input,select,textarea,[tabindex]')]
                .filter(e => !e.disabled && e.tabIndex>=0 && e.getClientRects().length), a=document.activeElement;
            if(!f.length){ ev.preventDefault(); return; }
            if(ev.shiftKey && (a===f[0] || !f.includes(a))){ ev.preventDefault(); f[f.length-1].focus(); }
            else if(!ev.shiftKey && (a===f[f.length-1] || !f.includes(a))){ ev.preventDefault(); f[0].focus(); }
        }
    }"
    x-id="['muni-cp']"
...
>
    ...
    <template x-teleport="body">
        <div x-show="open" x-cloak style="position:fixed;inset:0;z-index:250;display:flex;align-items:flex-start;justify-content:center;padding:12vh 20px 20px;">
            ...
            <div x-show="open" x-ref="panel" @keydown="atrapar($event)" x-transition:enter="muni-pop" x-transition:enter-start="muni-pop-0" x-transition:enter-end="muni-pop-1" role="dialog" aria-modal="true" aria-label="Paleta de comandos"
                ...
                <div style="display:flex;align-items:center;gap:10px;padding:14px 16px;border-bottom:1px solid var(--muni-border);">
                    <svg viewBox="0 0 20 20" fill="none" stroke="var(--muni-muted)" stroke-width="1.6" width="17" height="17" aria-hidden="true"><circle cx="9" cy="9" r="6"/><path d="M18 18l-4.5-4.5" stroke-linecap="round"/></svg>
                    <input x-ref="input" x-model="q" @input="active=0" @keydown.down.prevent="move(1)" @keydown.up.prevent="move(-1)" @keydown.enter.prevent="go()"
                           role="combobox" aria-autocomplete="list" :aria-expanded="results.length>0 ? 'true' : 'false'" :aria-controls="$id('muni-cp')"
                           :aria-activedescendant="results.length ? $id('muni-cp', 'o'+active) : null"
                ...
                <div style="max-height:52vh;overflow-y:auto;padding:6px;">
                    {{-- Sin resultados el listbox se oculta: vacío no tendría opciones. --}}
                    <div role="listbox" :id="$id('muni-cp')" aria-label="Resultados" x-show="results.length>0">
                        <template x-for="(item, i) in results" :key="i">
                            <a :href="item.url || '#'" @mouseenter="active=i" @click="open=false"
                               role="option" tabindex="-1" :id="$id('muni-cp', 'o'+i)" :aria-selected="active===i ? 'true' : 'false'"
                               class="muni-cp__opt" :class="active===i && 'muni-cp__opt--on'">
                                <span aria-hidden="true" style="width:6px;height:6px;border-radius:50%;background:var(--muni-accent);flex-shrink:0;box-shadow:var(--muni-glow);"></span>
...
@once
    <style>
        ...
        /* Opción activa: fondo de acento suave + barra izquierda, no solo un gris tenue. */
        .muni-cp__opt{display:flex;align-items:center;gap:11px;padding:10px 11px;border-radius:var(--muni-radius-sm);text-decoration:none;color:var(--muni-text);transition:background var(--muni-dur) var(--muni-ease);}
        .muni-cp__opt--on{background:var(--muni-accent-soft);box-shadow:inset 3px 0 0 var(--muni-accent);}
    </style>
@endonce
