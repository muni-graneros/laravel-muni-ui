 ...
 * abre está en `fuenteDeAlpine()` (tests/Helpers/FuentesEnDisco.php) y en
 * `abridorDeVitrina()`, acá abajo.
 *
...
/*
 * `fuenteDeAlpine()`, `fuenteDeFocus()` y `primeraFuenteEnDisco()` —de dónde sale
 * el Alpine que se hornea en la vitrina y por qué nunca de un CDN— vivían acá.
 * Ahora están en `tests/Helpers/FuentesEnDisco.php`, porque `GuardiaDeSesionTest`
 * también las usa y un archivo de prueba no puede tomar funciones de otro (lo
 * vigila `ArchivosDePruebaIndependientesTest`).
 */
